...
export function LeaderList() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [leaderData, adminData] = await Promise.all([
      ...
      console.error("Leader roster load failure:", error);
      showError("Failed to retrieve leader roster.");
    } finally {
      setLoading(false);
    }
  };

  const handleDeleteLeader = async (id: string, email: string) => {
    if (window.confirm(`CRITICAL: Permanently revoke leader privileges for ${email}?`)) {
      const previousLeaders = [...leaders];
      // Optimistic UI update
      setLeaders(prev => prev.filter(l => l.id !== id));
      showSuccess('Leader privileges revoked.');
      ...
      try {
        ...
        showError('LEADER REGISTRY ERROR: ' + (error.message || 'Verification failure'));
      }
    }
  };

  // Scoped list: regular Admins only see leaders they created
  const visibleLeaders = React.useMemo(() => {
    if (isSuperAdmin) return leaders;
    const currentUid = auth.currentUser?.uid;
    return leaders.filter(l => l.creatorId === currentUid);
  }, [leaders, isSuperAdmin]);

  const filteredLeaders = visibleLeaders.filter(leader => 
    ...
    <div className="space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Leaders</h1>
          <p className="text-slate-500 text-sm font-medium">Provision and manage leader access privileges across delivery teams.</p>
        </div>
      ...
      <div className="space-y-6">
        <div className="bg-indigo-50 border border-indigo-100 rounded-[1.5rem] p-5">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            <p className="text-xs font-black text-indigo-950">How do Leaders log in?</p>
          </div>
          <p className="text-[11px] text-indigo-700 leading-relaxed font-semibold">
            To register a new leader, click <strong>"Create Leader"</strong> above. Enter their name, email, and designation. Once registered, they should go to the sign-in screen, click <strong>"Sign Up / Set up Password"</strong>, enter their registered email, and configure a password. Once logged in, leaders can add developers and projects, and you will be able to see their stats by selecting them in the filters.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <AnimatePresence mode="popLayout">
              {filteredLeaders.map((leader) => {
                const creatorAdmin = admins.find(a => a.uid === leader.creatorId);
                return (
                  <motion.div
                    key={leader.id}
                    layout
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center mb-4 pt-2">
                      ...
                      <h3 className="font-black text-slate-900 text-center tracking-tight text-lg leading-tight">{leader.name}</h3>
                      {isSuperAdmin && creatorAdmin && (
                        <p className="text-[9px] text-slate-400 mt-1 font-bold">Created by: {creatorAdmin.name}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2 bg-slate-50/85 p-4 rounded-2xl border border-slate-100 flex-1 mb-4 shadow-inner group-hover:bg-white group-hover:border-indigo-50 transition-colors">
                      <div className="flex items-center gap-2.5 text-slate-500 overflow-hidden">
                        ...
                        <span className="text-[10px] font-bold truncate tracking-tight text-slate-600">{leader.designation || 'Team Leader'}</span>
                      </div>
                      <div className="flex items-center gap-2.5 text-slate-500 overflow-hidden">
                        <div className="p-1.5 bg-white rounded-lg border border-slate-200">
                          <Mail className="w-3 h-3 text-slate-400" />
                        </div>
                        <span className="text-[10px] font-bold truncate tracking-tight text-slate-600">{leader.email}</span>
                      </div>
                    </div>

                    <button
                      onClick={() => handleDeleteLeader(leader.id, leader.email)}
